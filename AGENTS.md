...
- Right Alt 长按提示必须通过独立 `shortcut-hint` IPC 在悬浮条窗口显示，主窗口不再渲染该提示卡；提示可见时隐藏胶囊，关闭逻辑只影响悬浮条展示，不回写主窗口 UI。
...
- 自由提问（`Right Alt + Space`）录音时胶囊栏显示 `请随意提出问题`；结果面板复用现有提示框视觉但位置不再固定为右上角；无选区时展示最终回答，有选区且原目标仍有效时直接替换选区，不再额外展示结果。
- 首页“最近结果”只展示最近一次最终转录/最终结果文字，不再展示实时语音状态、中间转写、复制按钮等旧结果卡 UI；实时状态只在悬浮胶囊栏展示。
